...
import { User, Tag, Calendar, AlertCircle, Search } from 'lucide-react';
...
const PublicTransactionList = ({ type, slug, currency, organization }) => {
  ...
  const getKey = (pageIndex, previousPageData) => {
    ...
    return `${import.meta.env.VITE_API_URL}/public/organizations/${slug}/${type === 'contributions' ? 'contributions' : 'expenses'}?page=${pageIndex + 1}&limit=20${searchParam}`;
  };
    ...
    <div className="w-full flex flex-col h-full bg-white dark:bg-gray-800">
      <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-700/50">
        <div className="relative">
          ...
          <input
            type="text"
            className="block w-full pl-10 pr-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg leading-5 bg-gray-50 dark:bg-gray-700/50 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 sm:text-sm transition-all"
      ...
      {isLoadingInitialData ? (
        ...
        >
                ...
                >
                  ...
                  {showExtra && (
                    <div className="mt-2 text-xs text-gray-600 dark:text-gray-400 flex flex-wrap gap-2">
                      {item.description && (
                        <span className="italic text-gray-500 w-full mb-1">"{item.description}"</span>
                      )}
          ...
          <div ref={loadMoreRef} className="p-4 text-center text-sm text-gray-500 dark:text-gray-400">
            {isValidating && !isReachingEnd && flatData.length > 0 ? 'Loading more...' : ''}
